...
class ContextMenu():
    ...
    def __init__(self, menu, viewbox_menu, tpg):
        ...
        for index in range(len(actions)):
            self.tpg.graphItem.ctrlMenu.removeAction(actions[index])
        for index in [0, 4, 6, 7, 8, 9, 10]:
            self.tpg.graphItem.ctrlMenu.addAction(actions[index])
    ...
    def ammend_context_menu(self):
        ...
        for line in line_controls:
            line.defaultWidget().layout().itemAt(2).widget().setValue(255*self.tpg.settings['line_settings'][str(key)]['line_alpha'])
            line.defaultWidget().layout().itemAt(4).widget().setValue(self.tpg.settings['line_settings'][str(key)]['line_width'])
            key += 1
